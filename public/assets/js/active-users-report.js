...
const activeUsersChartReportInit = () => {
  ...
  // 수기로 데이터 입력
  const seriesData = {
    primary: [5000, 5500, 4000, 6500, 5000, 7500],
    success: [2000, 2500, 1000, 3500, 4000, 5500],
    info: [3000, 1200, 1400, 1600, 1800, 2000]
  };
  ...
  return {
    ...
    xAxis: {
      ...
      axisTick: { show: true, length: 20, lineStyle: { color: utils.getGrays()['200'] }, interval: 0 },
      axisLabel: { color: utils.getGrays()['600'], formatter: value => value, align: 'center', fontSize: 11, interval: 0 }
    },
    yAxis: {
      type: 'value',
      position: 'right',
      ...
      axisLabel: { show: true, color: utils.getGrays()['600'], formatter: value => `${Math.round((value / 1000) * 10) / 10}k` },
      axisTick: { show: false },
      axisLine: { show: false }
    },
    series: [
